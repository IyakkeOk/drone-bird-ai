...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_convnext_tiny")
...
print(f"[INFO] ConvNeXt-Tiny results will be saved to: {RESULTS_DIR}")
...
def plot_confusion_matrix(cm, class_names, save_path):
    ...
    plt.title("Full Confusion Matrix (ConvNeXt-Tiny)")
...
def plot_error_distribution(errors, save_path):
    ...
    plt.title("Error Distribution per Class (ConvNeXt-Tiny)")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def plot_per_class_f1(f1_scores, save_path):
    ...
    plt.xlabel("Class Index")
    plt.ylabel("F1 Score (%)")
    plt.title("Per-Class F1 Scores (ConvNeXt-Tiny)")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
...
def plot_loss_curves(train_losses, val_accs, save_path_loss, save_path_acc):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Training Loss Curve (ConvNeXt-Tiny)")
    plt.legend()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Validation Accuracy Curve (ConvNeXt-Tiny)")
    plt.legend()
...
def main():
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    CONFIG_PATH = os.path.join(SCRIPT_DIR, "..", "configs", "cub_convnext_tiny.yaml")
    ...
    print("\nFinal Statistical Summary (ConvNeXt-Tiny):")
